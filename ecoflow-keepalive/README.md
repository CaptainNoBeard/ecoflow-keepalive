...
A lightweight, headless Playwright automation script designed specifically to run 24/7 on a Synology NAS via Docker.
...
- **Near-Zero CPU Footprint:** Aggressively blocks all images, stylesheets, fonts, and media. Furthermore, it injects a JavaScript throttle (`requestAnimationFrame`) to freeze the portal's heavy UI animations completely, bringing CPU usage down to almost 0%.
